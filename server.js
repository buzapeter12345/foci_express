...
app.delete("/jatekosma", async (req, res) => {
  try {
    const body = req.body;
    console.log(body);
    const toroltJatekosma = await Jatekosma.findByIdAndDelete({ _id: body.id});
...
app.delete("/jatekosfc", async (req, res) => {
  try {
    const body = req.body;
    console.log(body);
    const toroltJatekosfc = await Jatekosfc.findByIdAndDelete({ _id: body.id});
    res.status(200).json({ msg: "Sikeres játékos törlés!" });
  } catch (error) {
    res.status(500).json({ msg: "Valami hiba történt!" });
  }
